...
import org.bw.beact.parser.Node
import org.bw.beact.parser.NodeType
import org.bw.beact.parser.Parser
...
<:Login content="Login_content">
        ...
        contentParsed.forEach { node ->
            if (node.type == NodeType.BML_COMPONENT) {
                ...
                val fileSrc = getRealAttrValue(node.attrs["src"] ?: run {
                    error.addError(
                        org.bw.beact.errorSys.ErrorUnit(
                            org.bw.beact.errorSys.ErrorCodeType.INVALID_ATTRIBUTE,
                ...
                if (extension == "bml") {
                    ...
                    if (content == null) {
                        error.addError(
                            org.bw.beact.errorSys.ErrorUnit(
                                org.bw.beact.errorSys.ErrorCodeType.INVALID_FILE,
    ...
    private fun getTemplates(): List<Node> {
        val dirTemplates = "/templates"

        val json = BMLConverter::class.java.getResource("${dirTemplates}/templates.json")?.readText()
        val type = object : TypeToken<kotlin.Array<TemplateBML>>() {}.type
        val templateList = Gson().fromJson<kotlin.Array<TemplateBML>>(json, type)

        var components = listOf<Node>()

        templateList.forEach { template ->
            val contentString =
                BMLConverter::class.java.getResource("${dirTemplates}/${template.src}")?.readText() ?: ""
            val parsed = parser.parse(contentString)
    ...
    private fun calculateHTMLCode(
        ...
        when (node.type) {
            ...
            NodeType.UNKNOWN -> error.addError(
                org.bw.beact.errorSys.ErrorUnit(
                    org.bw.beact.errorSys.ErrorCodeType.INVALID_NODE,
    ...
    private fun getBML_VARIABLE(node: Node): String {
        ...
        if (isSet) {
            ...
            return variables[name_var] ?: run {
                error.addError(
                    org.bw.beact.errorSys.ErrorUnit(
                        org.bw.beact.errorSys.ErrorCodeType.UNDEFINED_VARIABLE,
    ...
    private fun getBML_FUNCTION(node: Node): String {
        ...
        when (node.name) {
            "${"$"}import" -> {
                ...
                when (extension) {
                    ...
                    "css" -> HTML_output = "<style>${content}</style>"
                        ...
                        error.addError(
                            org.bw.beact.errorSys.ErrorUnit(
                                org.bw.beact.errorSys.ErrorCodeType.INVALID_FILE_EXTENSION,
            ...
            "${"$"}foreach" -> {
                ...
                error.addError(
                    org.bw.beact.errorSys.ErrorUnit(
                        org.bw.beact.errorSys.ErrorCodeType.INVALID_FUNCTION,
